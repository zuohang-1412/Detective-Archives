...
- 150 位正式目录人物及 160 部可浏览代表作品数据，109 条图鉴记录均已关联正式档案；每位已发布人物至少关联一部带有效正版/权威入口的作品，109 条原始推荐标签全部可直达对应作品、合集或系列。
...
- 登录前协议/隐私确认、协议版本与同意时间留存、版本变更后的强制重新确认、微信平台隐私状态查询与授权同步、个人数据 JSON 导出，以及撤销身份、会话和个人内容的账号注销流程；协议待确认或账号受限期间仍可导出、退出和注销，开发工具固定使用明确的稳定基础库版本。
...
- 基础设施演练记录器：监控必须证明健康、首次开单、重复故障去重和恢复关单；离站备份必须证明真实制品摘要、密钥代次、计时隔离恢复及 150/161/109/109 数据库基线（161 部作品记录含 1 条补偿归档历史），两个门禁均拒绝手工布尔值。
- 产品 PRD、页面规划和技术架构文档。
- API 自动化测试。

首版目录已形成 150 位人物、160 部公开作品、161 条有效正版/馆藏入口和 109 条图鉴推荐直达的数据闭环；已发布人物均有权威来源和代表作品，当前外链巡检与证据复核为 107 条自动健康、54 条人工确认、0 条复核积压、0 条确认失效。AI 视频属于 P2 范围，详见 `docs/PRD-v0.1.md`。
